...
const {
  Gate6VerifierError, buildExperimentConfig, createHarnessSampleReceipt, createPreflightRecord,
  createRawReport, createRuntimeIdentity, evaluatePromotion, evaluateStrategyThresholds,
  loadFrozenBenchmark, verifyEvidenceDirectory, writeEvidencePackage
...
function fixtureBundle(options = {}) {
  ...
  for (const task of built.frozen.tasks) {
    ...
    for (const strategy of built.config.contextStrategySemantics.strategies) for (let repetition = 1; repetition <= built.config.repetitions; repetition += 1) {
      ...
      receiptInputs.push({ observation, harnessReport: harnessReport(task, strategy, options.receiptOverride?.({ task, strategy, repetition, observation }) ?? {}) });
    }
  }
...
function main() {
  ...
  test("harness FAIL plus observation endToEndAccepted true is rejected", () => { const fixture = fixtureBundle(); const observations = fixture.rawReport.observations.map((row) => ({ ...row })); const old = fixture.rawReport.sampleReceipts[0]; const task = fixture.frozen.tasks.find((t) => t.taskId === old.taskId); const report = harnessReport(task, old.strategy, { status: "rejected", failureCode: "TEST_FAILURE", failureDomain: "verification", metrics: { testsPassed: false, acceptancePassed: false } }); const receipts = fixture.rawReport.sampleReceipts.map((r, i) => i === 0 ? createHarnessSampleReceipt({ observation: observations[0], harnessReport: report }) : r); expectReject(() => rebuild(fixture, observations, receipts), "GATE6_VERIFY_RECEIPT_END_TO_END_ACCEPTED_MISMATCH"); });
  test("unauthorized mutation receipt plus scopeViolation false is rejected", () => { const fixture = fixtureBundle(); const observations = fixture.rawReport.observations.map((row) => ({ ...row })); const old = fixture.rawReport.sampleReceipts[0]; const task = fixture.frozen.tasks.find((t) => t.taskId === old.taskId); const report = harnessReport(task, old.strategy, { status: "rejected", failureCode: "UNAUTHORIZED_FILE_MUTATION", failureDomain: "policy", unauthorizedFiles: ["src/side-effect.js"], metrics: { unauthorizedFileMutation: true, acceptancePassed: false } }); const receipts = fixture.rawReport.sampleReceipts.map((r, i) => i === 0 ? createHarnessSampleReceipt({ observation: observations[0], harnessReport: report }) : r); expectReject(() => rebuild(fixture, observations, receipts), "GATE6_VERIFY_RECEIPT_SCOPE_VIOLATION_MISMATCH"); });
