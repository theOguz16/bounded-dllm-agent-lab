...
} = require("../../scripts/lib/gate6-oracle-scorer.cjs");
...
function fixtureBundle(options = {}) {
  ...
  for (const task of built.frozen.tasks) {
    ...
    for (const strategy of built.config.contextStrategySemantics.strategies) for (let repetition = 1; repetition <= built.config.repetitions; repetition += 1) {
      ...
      receiptInputs.push({ observation, task, oracle, selectionEvidence: selection, harnessReport: harnessReport(task, strategy, options.receiptOverride?.({ task, oracle, strategy, repetition, observation }) ?? {}) });
    }
  }
...
function rehashReceipt(receipt) { receipt.selectionEvidenceHash = hashCanonical(receipt.selectionEvidence); receipt.oracleVerificationHash = hashCanonical(receipt.oracleVerification); const { receiptHash: _old, ...core } = receipt; receipt.receiptHash = hashCanonical(core); return receipt; }
function main() {
  ...
  test("wrong selection plus forged strictOracleSuccess true is rejected after valid rehash", () => { const fixture = fixtureBundle(); const observations = fixture.rawReport.observations.map((row) => ({ ...row })); const receipts = fixture.rawReport.sampleReceipts.map((r) => structuredClone(r)); const target = receipts.find((receipt) => receipt.oracleVerification.strictOracleSuccess && receipt.selectionEvidence.selectedSymbols.length > 0); assert(target); const observation = matchingObservation(observations, target); target.selectionEvidence.selectedSymbols = []; target.oracleVerification = { ...target.oracleVerification, strictOracleSuccess: true, exactSymbolSuccess: true, symbolTruePositiveCount: target.oracleVerification.symbolRequiredCount, symbolPredictedCount: target.oracleVerification.symbolRequiredCount }; observation.strictOracleSuccess = true; observation.exactSymbolSuccess = true; observation.symbolTruePositiveCount = target.oracleVerification.symbolRequiredCount; observation.symbolPredictedCount = target.oracleVerification.symbolRequiredCount; rehashReceipt(target); expectReject(() => rebuild(fixture, observations, receipts), "GATE6_VERIFY_ORACLE_SCORE_RECOMPUTE_MISMATCH"); });
