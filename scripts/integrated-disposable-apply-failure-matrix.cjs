...
const gitEnv = {
  ...process.env,
  GIT_AUTHOR_NAME: "AC3 Fixture",
  GIT_AUTHOR_EMAIL: "ac3@example.invalid",
  GIT_COMMITTER_NAME: "AC3 Fixture",
  GIT_COMMITTER_EMAIL: "ac3@example.invalid"
};
...
(async () => {
  ...
  try {
    ...
    await check(
      "validation workspace overlap blocks before X4 claim",
      async () => {
        const value = await fixture();
        value.input.validationWorkspaceParentPath =
          value.repositoryPath;

        const result =
          await runIntegratedDisposableApply(
            value.input
          );
    ...
    await check(
      "symlinked validation workspace blocks before X4",
      async () => {
        const value = await fixture();
        ...
        const link = path.join(
          os.tmpdir(),
          `ac3-workspace-link-${Date.now()}-${Math.random()}`
        );
    ...
    await check(
      "pre-existing isolated workspace blocks before X4",
      async () => {
        const value = await fixture();
        const key =
          value.input.bindingReceipt
        ...
        const workspace = path.join(
          value.input
            .validationWorkspaceParentPath,
          `controlled-post-apply-${key}.partial`
        );
        fs.mkdirSync(
          workspace,
          {
    ...
    await check(
      "pre-existing validation transaction blocks before X4",
      async () => {
        const value = await fixture();
        const key =
          value.input.bindingReceipt
    ...
    await check(
      "oversized X5 output bound blocks before X4",
      async () => {
        const specification = {
          commands: [
            {
    ...
    await check(
      "objective mismatch blocks before X4",
      async () => {
        const value = await fixture();

        value.input.acceptanceContract =
          createAcceptanceCriteriaContract({
            taskId:
              "ac3-wrong-objective",
            objectiveHash:
        ...
        assert.equal(
          result.summary.objectiveMatched,
          false
        );
        assert.equal(
          result.summary.applyCallCount,
    ...
    await check(
      "Phase V artifact mismatch blocks before X4",
      async () => {
        const value = await fixture();

        value.input.phaseVExecutionVerification = {
          ...value.input
            .phaseVExecutionVerification,
    ...
    await check(
      "concurrent coordinator calls produce one finalized result",
      async () => {
        const value = await fixture();
    ...
    await check(
      "repository drift makes integrated receipt stale",
      async () => {
        const value = await fixture();
        const result =
          await runIntegratedDisposableApply(
        ...
        const verified =
          await verifyIntegratedDisposableApplyReceipt({
            receipt:
              result.receipt,
            bindingReceipt:
              value.input
            ...
            acceptanceContract:
              value.input
                .acceptanceContract,
            preflightCoverageReceipt:
              result.preflightAcceptance
                .receipt,
            finalCoverageReceipt:
              result.finalAcceptance
                .receipt,
            finalExecutionEvidence:
              result.finalExecutionEvidence,
            applyReceipt:
              result.applyResult
                .receipt,
            postApplyFinalReceipt:
              result.postApplyValidation
                .finalReceipt,
            executionAuthorization:
              value.input.bindingInput
                .executionAuthorization,
            bindingInput:
              value.input
                .bindingInput,
        ...
        assert.equal(
          verified.downstreamEligible,
          false
        );
        assert.equal(
          verified.repositoryStateMatched,
          false
        );
      }
    );
    ...
    console.log(
      "integrated disposable apply failure matrix passed (18 checks)"
    );
  } finally {
