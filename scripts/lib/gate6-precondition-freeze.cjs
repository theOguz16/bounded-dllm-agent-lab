...
function validateSpec(preconditions, tasks) {
  if (!isPlainObject(preconditions) || preconditions.schemaVersion !== PRECONDITION_SCHEMA || !Array.isArray(preconditions.entries)) {
    fail("GATE6_PRECONDITION_SPEC_INVALID");
  }
  ...
  for (const entry of preconditions.entries) {
    ...
    if (!isPlainObject(entry.acceptanceCheck) || entry.acceptanceCheck.type !== "repository_test_suite" || entry.acceptanceCheck.command !== "npm test") {
...
function validateAttestations({ preconditions, tasks, repositories, attestations }) {
  ...
  for (const attestation of attestations) {
    ...
    if (attestation.acceptanceCommand !== "npm test" || !Array.isArray(attestation.results)) fail("GATE6_PRECONDITION_ATTESTATION_COMMAND_INVALID", attestation.repositoryId);
    const { attestationHash, ...core } = attestation;
    if (attestationHash !== hash(core)) fail("GATE6_PRECONDITION_ATTESTATION_HASH_INVALID", attestation.repositoryId);
    for (const result of attestation.results) {
      ...
      if (!isPlainObject(result.evidence) || !Array.isArray(result.evidence.requiredTestFiles) || !Array.isArray(result.evidence.behavioralChecks) || result.evidence.behavioralChecks.length === 0) {
        fail("GATE6_PRECONDITION_BEHAVIOR_EVIDENCE_MISSING", result.taskId);
      }
      if (entry.baselineExpected === "pass") {
        if (result.faultInjection !== null || Object.prototype.hasOwnProperty.call(result, "injectedAcceptanceExitCode")) {
          fail("GATE6_PRECONDITION_NO_CHANGE_RESULT_INVALID", result.taskId);
        }
      } else {
