...
function bool(value, code, detail) {
  if (typeof value !== "boolean") fail(code, detail);
  return value;
}
function count(value, detail) {
  if (!Number.isSafeInteger(value) || value < 0) fail("GATE6_VERIFY_RECEIPT_ORACLE_VERIFICATION_INVALID", detail);
  return value;
}
...
function createHarnessSampleReceipt({ observation, harnessReport, oracleVerification }) {
  ...
  const core = {
    ...
    commitSha: harnessReport.commitSha,
...
function createRawReport({ rootPath, config, experimentConfigHash, observations, repositorySnapshots, sampleReceipts }) {
  ...
  const raw = {
    ...
    sampleReceiptSetHash: base.hashCanonical(normalizedReceipts.map((receipt) => receipt.receiptHash)),
...
function buildEvidence({ rootPath, rawReport, runtimeIdentity, preflight }) {
  ...
  return deepFreeze({
    ...
    sampleReceiptSetHash: rawReport.sampleReceiptSetHash,
    provenance: {
...
function ensureFresh(outputDir) {
  ...
  for (const filename of base.EVIDENCE_FILENAMES) if (existsSync(path.join(outputDir, filename))) fail("GATE6_VERIFY_EVIDENCE_ALREADY_EXISTS", path.join(outputDir, filename));
}
