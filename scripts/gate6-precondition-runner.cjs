...
const RUNNER_VERSION = "gate6-precondition-runner/v3";
...
const FAULT_TARGET_OVERRIDES = Object.freeze({
  "external.commander-suggestion-dependency": { path: "lib/suggestSimilar.js", symbol: "suggestSimilar" }
});

const PROBE_OVERRIDES = Object.freeze({
  "external.escape-string-regexp-hyphen-boundary": `import assert from 'node:assert/strict'; import escapeStringRegexp from './index.js'; assert.equal(escapeStringRegexp('-'),String.raw\`\\x2d\`); assert.doesNotThrow(()=>new RegExp(escapeStringRegexp('-'),'u'));`
});
...
function validateCoverage(dataset) {
  ...
  for (const entry of dataset.preconditions.entries) {
    ...
    if (!entry.acceptanceCheck || entry.acceptanceCheck.type !== "node_probe" || entry.acceptanceCheck.probeId !== entry.taskId) fail("GATE6_PRECONDITION_ACCEPTANCE_CHECK_INVALID", entry.taskId);
    ...
    if (expected === "fail" && (!entry.faultInjection || entry.faultInjection.type !== "rename_primary_required_symbol")) fail("GATE6_PRECONDITION_MUTATION_INJECTION_REQUIRED", entry.taskId);
...
function anchorVariants(anchor) {
  return [...new Set([anchor, anchor.replaceAll("\\", "\\\\"), anchor.replace(/\r/g, "\\r").replace(/\n/g, "\\n")])];
}
function verifyEvidence(repoPath, task, oracle) {
  const files = candidateTestFiles(task, oracle);
  for (const relative of files) if (!existsSync(path.join(repoPath, relative))) fail("GATE6_PRECONDITION_TEST_FILE_MISSING", `${task.taskId}:${relative}`);
  for (const anchor of oracle.requiredTestAnchors) {
    const variants = anchorVariants(anchor);
    let found = false;
    for (const relative of files) {
      const source = readFileSync(path.join(repoPath, relative), "utf8");
      if (variants.some((variant) => source.includes(variant))) { found = true; break; }
    }
  ...
  return { testFiles: [...files].sort(compareText), requiredTestAnchors: [...oracle.requiredTestAnchors].sort(compareText), behavioralChecks: [...oracle.behavioralChecks] };
}
...
function faultCandidates(oracle, task) {
  const override = FAULT_TARGET_OVERRIDES[task.taskId];
  if (override) return [override];
  const result = [];
  for (const relative of oracle.requiredImplementationFiles ?? []) for (const symbol of oracle.requiredSymbols ?? []) result.push({ path: relative, symbol });
  return result;
}
function applyFaultInjection(repoPath, oracle, task) {
  for (const { path: relative, symbol } of faultCandidates(oracle, task)) {
    ...
    for (const pattern of declarationPatterns(symbol)) {
      ...
      let mutatedMatch = match[0].replace(symbol, replacementSymbol);
      const defaultExportRemoved = /^export\s+default\s+/.test(mutatedMatch);
      if (defaultExportRemoved) mutatedMatch = mutatedMatch.replace(/^export\s+default\s+/, "");
      ...
      return {
        ...
        symbol,
        replacementSymbol,
        defaultExportRemoved,
        beforeBlobSha,
        beforeContentHash,
        afterContentHash,
        injectionId: sha256(JSON.stringify({ commitSha: task.commitSha, path: relative, beforeBlobSha, beforeContentHash, afterContentHash, symbol, replacementSymbol, defaultExportRemoved }))
      };
    }
  }
...
function installExactRuntimeDependencies(repoPath, repositoryId) {
  ...
  mustRun("npm", ["install", "--no-save", "--package-lock=false", "--ignore-scripts", "--omit=dev", "--no-audit", "--no-fund", ...profile.dependencies], { cwd: repoPath, timeout: 10 * 60 * 1000 });
...
function walk(directory, callback) {
  for (const name of readdirSync(directory)) {
    ...
    if (stat.isDirectory()) walk(full, callback); else callback(full);
  }
}
function transformTypeScript(repoPath, repositoryId) {
  ...
  walk(sourceRoot, (full) => {
    ...
    let code;
    try { code = stripTypeScriptTypes(readFileSync(full, "utf8"), { mode: "transform", sourceMap: false }); }
    catch (error) { fail("GATE6_PRECONDITION_TYPESCRIPT_TRANSFORM_FAILED", `${repositoryId}:${relative}:${error.message}`); }
    code = code.replace(/(["'])((?:\.\.?\/)[^"']+)\.ts\1/g, "$1$2.js$1");
    writeFileSync(target, code);
  });
}
function prepareRuntime(repoPath, repositoryId) { transformTypeScript(repoPath, repositoryId); }
function runProbe(repoPath, taskId) {
  const probe = PROBE_OVERRIDES[taskId] ?? getProbe(taskId);
...
function main() {
  ...
  for (const task of tasks) {
    ...
    if (cleanProbe.exitCode !== 0) fail("GATE6_PRECONDITION_CLEAN_PROBE_FAILED", `${task.taskId}\n${cleanProbe.stderrTail}\n${cleanProbe.stdoutTail}`);
    if (entry.baselineExpected === "pass") {
      results.push({ taskId: task.taskId, repositoryId: task.repositoryId, commitSha: task.commitSha, probeId: entry.acceptanceCheck.probeId, baselineExpected: "pass", baselineObserved: "pass", cleanAcceptanceExitCode: 0, injectedAcceptanceExitCode: null, faultInjection: null, evidence });
      continue;
    }
    resetRepository(repoPath, repository.commitSha);
    ...
    if (injectedProbe.exitCode === 0) fail("GATE6_PRECONDITION_INJECTED_PROBE_DID_NOT_FAIL", task.taskId);
    results.push({ taskId: task.taskId, repositoryId: task.repositoryId, commitSha: task.commitSha, probeId: entry.acceptanceCheck.probeId, baselineExpected: "fail", baselineObserved: "fail", cleanAcceptanceExitCode: 0, injectedAcceptanceExitCode: injectedProbe.exitCode, faultInjection: injection, evidence });
  }
