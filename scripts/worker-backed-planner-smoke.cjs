...
const SUITE_NAME = "phase-p-worker-backed-planner-smoke";

const fixture = {
  caseId: "phase-p-planner-safe-helper",
  task: "Plan a bounded change to add a small helper function.",
...
function configFromEnv() {
  return {
    upstreamUrl: process.env.WORKER_PLANNER_UPSTREAM_URL || "",
    modelId: process.env.WORKER_PLANNER_MODEL_ID || "qwen2.5-coder-7b",
    timeoutMs: readIntegerEnv("WORKER_PLANNER_TIMEOUT_MS", 120000),
    maxTokens: readIntegerEnv("WORKER_PLANNER_MAX_TOKENS", 256),
    required: process.env.WORKER_PLANNER_REQUIRED === "1",
    outDir: process.env.WORKER_PLANNER_OUT_DIR || path.join("reports", "worker-backed-planner-smoke")
  };
}
...
function buildPlannerMessages(testFixture = fixture) {
  const example = {
    ...
    claims: [
      {
        type: "planned_step",
        description: "Add the helper only inside the allowed file."
      }
    ],
...
function writeReport(report, config) {
  ...
  const jsonPath = path.join(outDir, `${timestamp}-worker-backed-planner-smoke.json`);
  const markdownPath = path.join(outDir, `${timestamp}-worker-backed-planner-smoke.md`);
...
async function callOpenAiCompatibleEndpoint(config, messages) {
  ...
  try {
    ...
    if (!response.ok) {
      throw new Error(`Planner endpoint returned HTTP ${response.status}: ${preview(text, 500)}`);
    }
...
async function run() {
  ...
  if (!config.upstreamUrl) {
    ...
    report.validation.issues = [
      {
        code: "invalid_shape",
        message: "WORKER_PLANNER_UPSTREAM_URL is not configured."
      }
    ];
    return writeReport(report, config);
  }

  const messages = buildPlannerMessages(fixture);
