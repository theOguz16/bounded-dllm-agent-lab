...
const { createRepository, sourceOriginal, sourceChanged, fakeDiscoveryAdapter,
...
async function main() {
  ...
  const checkHash = sha256(await fs.readFile(checker));
  const criterion = { criterionId: "calculate.multiplies-by-three", checkHash };
  ...
  const counters = { discovery: 0, execution: 0, baseline: 0 };
  ...
  for (const variant of ["valid", "missing", "corrupt", "wrong_candidate"]) {
    ...
    const result = await compare.compareCodexCommand(
      { task: "Make calculate multiply by three." }, repository,
      {
        adapter, model: "fixture-model",
        prepareValidationSubstrate: async () => ({
          version: substrateModule.COMPARE_VALIDATION_SUBSTRATE_VERSION,
          image: substrateModule.COMPARE_VALIDATION_IMAGE,
          dependencyRoot, dependencySnapshotHash: runtime.hashCanonicalJson("empty-fixture-dependencies"),
          prepared: false
        }),
        trustedBehavior: async ({ workspacePath, candidateTreeHash, taskId, taskHash,
          ...
          if (variant === "wrong_candidate") {
            ...
            candidateForReceipt = await executeAcceptance(
              checker, otherCandidate, evidenceDirectory, "other-candidate", snapshot
            );
    ...
    assert.equal(callbackInvocations, 1, `${variant}: trusted host not invoked`);
  ...
  assert.equal(snapshot(repository), sourceTreeHash);
  assert.equal(counters.discovery, 4);
  assert.equal(counters.baseline, 4);
  assert.ok(counters.execution >= 2, "real bounded candidate execution was not observed");
  process.stdout.write(`${JSON.stringify({ result: "PASS", sourceCommitSha, sourceTreeHash,
    counters, reports, evidenceDirectory, fixtureRoot: parent }, null, 2)}\n`);
}
