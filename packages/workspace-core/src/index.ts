...
export type WorkspaceTask = {
  ...
  /**
   * Explicit task-level authority.
   * Example: "This task is allowed to modify packages/workspace-core only."
   */
...
export type WorkspaceScope = {
  /**
   * Paths or logical regions the agent is allowed to reason/write inside.
   */
  allowed: string[];

  /**
   * Paths or logical regions the agent must not touch.
   */
  forbidden: string[];

  /**
   * Files touched by the current patch intent.
   */
  changedFiles: string[];
};
...
export type WorkspaceAuthority = {
  /**
   * Known authority facts extracted from task, policy, repo metadata or user input.
   */
  facts: string[];

  /**
   * Missing authority signals that should block unsafe inference.
   */
  missingRules: string[];
};
...
export type WorkspacePatchIntent = {
  /**
   * Raw patch/diff text. Product-runtime can decide how much of this is exposed
   * to each role view.
   */
  rawDiff: string;
...
export type WorkspaceClaim = {
  ...
  /**
   * Claim'in hangi workspace revision'ına göre yazıldığını tutar.
   * Merge aşamasında stale claim kontrolü için temel alan budur.
   */
  baseRevision: number;

  /**
   * İleride orchestrator bu alanı kullanarak role-specific write permission
   * denetimi yapacak.
   */
...
export type WorkspaceVerifierResult = {
  ...
  /**
   * Verifier bütün cevabı tekrar yazdırmak yerine hangi workspace region'ının
   * problemli olduğunu işaretler. Remask loop bunun üzerine kurulacak.
   */
...
export type SharedSemanticWorkspace = {
  ...
  /**
   * revision workspace mutation counter'dır.
   * Schema version değildir; runtime state değiştikçe artar.
   */
  revision: number;
  ...
  /**
   * Trace bu mimarinin kalbidir.
   * Agent'lar ortak chat'e mesaj yazmaz; ortak state üzerinde izlenebilir
   * mutation event'leri üretir.
   */
...
export function addVerifierResult(
  ...
  return withTraceEvent({
    ...
    relatedIds: [
      verifierResult.id,
      verifierResult.checkName,
      ...verifierResult.evidenceIds,
...
function assertWorkspaceShape(value: SharedSemanticWorkspace): void {
  ...
  if (!value.roleViews || typeof value.roleViews !== "object") {
    throw new Error("Invalid SharedWorkspace payload: roleViews must be an object.");
  }
