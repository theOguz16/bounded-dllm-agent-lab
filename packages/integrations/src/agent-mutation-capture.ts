...
} from "../../product-runtime/src/runtime-contract-foundation.js";
import {
...
function fail(
  code: AgentMutationCaptureErrorCode,
  message: string,
  file?: string
): never {
...
function parseNulFields(bytes: Buffer): string[] {
  ...
  if (bytes.at(-1) !== 0) {
    fail(
      "agent_mutation_git_diff_invalid",
      "Git NUL-delimited output ended without a terminating NUL byte."
    );
  }
  return bytes
...
function parseRawDiff(bytes: Buffer): GitRawChange[] {
  ...
  for (let index = 0; index < fields.length; ) {
    ...
    if (match === null) {
      fail(
        "agent_mutation_git_diff_invalid",
        `Unexpected raw Git diff header: ${JSON.stringify(header ?? "")}.`
      );
    }

    const path = fields[index++];
    if (path === undefined || path.length === 0) {
      fail("agent_mutation_git_diff_invalid", "Raw Git diff is missing a changed path.");
    }
    ...
    if (status === "R" || status === "C") {
      ...
      if (secondPath === undefined || secondPath.length === 0) {
        fail(
          "agent_mutation_git_diff_invalid",
          `Raw Git ${status} entry is missing its destination path.`
        );
      }
    }
...
function parseBaselineTree(bytes: Buffer): BaselineTreeEntry[] {
  ...
  return fields.map((field) => {
    ...
    if (match === null) {
      fail(
        "agent_mutation_baseline_invalid",
        `Unexpected Git baseline tree entry: ${JSON.stringify(field)}.`
      );
    }
...
function validateManifest(
  ...
  if (
    ...
    fail(
      "agent_mutation_source_manifest_invalid",
      "A valid pre-agent disposable workspace manifest is required."
    );
  }
  ...
  for (const entry of manifest.files) {
    ...
    if (!SHA256_HEX.test(entry.sourceHash)) {
      fail(
        "agent_mutation_source_manifest_invalid",
        `Manifest source hash is invalid for ${path}.`,
        path
      );
    }
    if (!Number.isSafeInteger(entry.bytes) || entry.bytes < 0) {
      fail(
        "agent_mutation_source_manifest_invalid",
        `Manifest byte count is invalid for ${path}.`,
        path
      );
    }
...
async function verifyWorkspaceGitBoundary(
  ...
  try {
    ...
    fail(
      "agent_mutation_workspace_invalid",
      "Disposable agent workspace must contain its own .git directory."
    );
  }
  if (!dotGitStat.isDirectory() || dotGitStat.isSymbolicLink()) {
    fail(
      "agent_mutation_workspace_invalid",
      "Disposable agent workspace .git must be a regular directory."
    );
  }

  const gitDirOutput = (
    await runGitText(workspaceRealPath, ["rev-parse", "--absolute-git-dir"], environment)
  ).trim();
  ...
  if (gitDirRealPath !== expectedGitDirRealPath) {
    fail(
      "agent_mutation_workspace_invalid",
      "Agent workspace Git directory points outside the disposable workspace."
    );
  }

  const commonDirOutput = (
    await runGitText(workspaceRealPath, ["rev-parse", "--git-common-dir"], environment)
  ).trim();
  ...
  if (commonDirRealPath !== gitDirRealPath) {
    fail(
      "agent_mutation_workspace_invalid",
      "Agent workspace Git common directory points outside its own .git directory."
    );
  }

  const remotes = (await runGitText(workspaceRealPath, ["remote"], environment)).trim();
  if (remotes.length > 0) {
    fail(
      "agent_mutation_workspace_invalid",
      "Agent workspace Git repository must not configure remotes."
    );
  }

  try {
    const alternates = await readFile(
      join(gitDirRealPath, "objects", "info", "alternates"),
      "utf8"
    );
    if (alternates.trim().length > 0) {
      fail(
        "agent_mutation_workspace_invalid",
        "Agent workspace Git repository must not use alternate object directories."
      );
    }
  } catch (error) {
...
async function findVerifiedBaselineCommit(
  workspaceRealPath: string,
  manifestByPath: ReadonlyMap<
    string,
    DisposableAgentWorkspaceManifest["files"][number]
  >,
  environment: NodeJS.ProcessEnv
): Promise<string> {
  const roots = (
  ...
  if (roots.length !== 1 || !/^[0-9a-f]{40,64}$/.test(roots[0]!)) {
    fail(
      "agent_mutation_baseline_invalid",
      "Agent workspace must have exactly one reachable root baseline commit."
    );
  }
  const baselineCommit = roots[0]!;

  const tree = parseBaselineTree(
    await runGitBuffer(
      workspaceRealPath,
  ...
  if (tree.length !== manifestByPath.size) {
    fail(
      "agent_mutation_source_manifest_mismatch",
      "Workspace Git baseline path set differs from the pre-agent manifest."
    );
  }
  ...
  for (const treeEntry of tree) {
    ...
    if (manifestEntry === undefined || seen.has(path)) {
      fail(
        "agent_mutation_source_manifest_mismatch",
        `Workspace Git baseline contains an unexpected path: ${path}.`,
        path
      );
    }
    seen.add(path);
    ...
    const baselineBytes = await runGitBuffer(
      workspaceRealPath,
      ["cat-file", "blob", treeEntry.object],
      environment
    );
    if (
      baselineBytes.byteLength !== manifestEntry.bytes ||
      sha256(baselineBytes) !== manifestEntry.sourceHash
    ) {
      fail(
        "agent_mutation_source_manifest_mismatch",
        `Workspace Git baseline content differs from the pre-agent manifest for ${path}.`,
        path
      );
    }
  }
  ...
  for (const path of manifestByPath.keys()) {
    if (!seen.has(path)) {
      fail(
        "agent_mutation_source_manifest_mismatch",
        `Workspace Git baseline is missing manifest path ${path}.`,
        path
      );
    }
  }
...
function rejectUnsupportedChange(change: GitRawChange): void {
  ...
  const destination =
    change.secondPath === undefined ? undefined : canonicalMutationPath(change.secondPath);

  switch (change.status) {
    case "A":
      fail("agent_mutation_added_unsupported", `Added files are not supported: ${path}.`, path);
    case "D":
      fail("agent_mutation_deleted_unsupported", `Deleted files are not supported: ${path}.`, path);
    case "R":
      fail(
        "agent_mutation_renamed_unsupported",
        `Renamed files are not supported: ${path} -> ${destination ?? "?"}.`,
        path
      );
    case "C":
      fail(
        "agent_mutation_copied_unsupported",
        `Copied files are not supported: ${path} -> ${destination ?? "?"}.`,
        path
      );
    case "T":
      fail(
        change.newMode === "120000" || change.oldMode === "120000"
          ? "agent_mutation_symlink_unsupported"
          : "agent_mutation_file_type_unsupported",
        `File type changes are not supported: ${path}.`,
        path
      );
    case "M":
      return;
    default:
      fail(
        "agent_mutation_git_diff_invalid",
        `Unsupported Git diff status ${change.status} for ${path}.`,
        path
      );
  }
}
...
async function readCurrentTextFile(
  ...
  if (!isWithin(workspaceRealPath, destination)) {
    fail("agent_mutation_path_escape", `Changed path escapes workspace: ${path}.`, path);
  }
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    try {
      ...
      if ((error as NodeJS.ErrnoException).code === "ENOENT") {
        fail(
          "agent_mutation_deleted_unsupported",
          `Changed file no longer exists: ${path}.`,
          path
        );
      }
      throw error;
    ...
    if (metadata.isSymbolicLink()) {
      fail("agent_mutation_symlink_unsupported", `Symlinks are not supported: ${path}.`, path);
    }
    if (index < segments.length - 1 && !metadata.isDirectory()) {
      fail(
        "agent_mutation_file_type_unsupported",
        `Changed path has a non-directory ancestor: ${path}.`,
        path
      );
    }
    if (index === segments.length - 1 && !metadata.isFile()) {
      fail(
        "agent_mutation_file_type_unsupported",
        `Changed target must be a regular file: ${path}.`,
        path
      );
    }
  }
  ...
  try {
    ...
    if (!metadata.isFile()) {
      fail(
        "agent_mutation_file_type_unsupported",
        `Changed target must remain a regular file: ${path}.`,
        path
      );
    }
    if (metadata.size > MUTATION_LIMITS.maxFileBytes) {
      fail(
        "agent_mutation_file_limit_exceeded",
        `Changed file exceeds ${MUTATION_LIMITS.maxFileBytes} bytes: ${path}.`,
        path
      );
    }

    const expectedPermissions = Number.parseInt(expectedGitMode.slice(-3), 8);
    if ((metadata.mode & 0o777) !== expectedPermissions) {
      fail(
        "agent_mutation_mode_change_unsupported",
        `Filesystem mode does not match Git diff mode for ${path}.`,
        path
      );
    }
    ...
    if (offset !== bytes.length) {
      fail(
        "agent_mutation_git_diff_invalid",
        `Changed file size changed during capture: ${path}.`,
        path
      );
    }
    ...
    if (bytes.includes(0)) {
      fail(
        "agent_mutation_binary_unsupported",
        `NUL-containing files are not supported: ${path}.`,
        path
      );
    }
    for (const byte of bytes) {
      if (byte < 0x20 && byte !== 0x09 && byte !== 0x0a && byte !== 0x0d) {
        fail(
          "agent_mutation_binary_unsupported",
          `Control-byte content is not supported: ${path}.`,
          path
        );
      }
    }
    ...
    try {
      ...
      fail(
        "agent_mutation_binary_unsupported",
        `Changed file is not valid UTF-8: ${path}.`,
        path
      );
    }
    if (!Buffer.from(content, "utf8").equals(bytes)) {
      fail(
        "agent_mutation_binary_unsupported",
        `Changed file does not round-trip as UTF-8: ${path}.`,
        path
      );
    }
...
export async function captureAgentMutations(
  ...
  if (
    ...
    fail(
      "agent_mutation_workspace_invalid",
      "workspacePath must identify a disposable agent workspace."
    );
  }
  ...
  try {
    ...
    fail(
      "agent_mutation_workspace_invalid",
      "workspacePath cannot be resolved."
    );
  }
  ...
  if (!workspaceStat.isDirectory() || workspaceStat.isSymbolicLink()) {
    fail(
      "agent_mutation_workspace_invalid",
      "workspacePath must resolve to a regular directory."
    );
  }

  return withIsolatedGitHome(async (environment) => {
    await verifyWorkspaceGitBoundary(workspaceRealPath, environment);
    const baselineCommit = await findVerifiedBaselineCommit(
      workspaceRealPath,
      manifestByPath,
      environment
    );

    const untrackedPaths = parseNulFields(
      await runGitBuffer(
        workspaceRealPath,
    ...
    if (untrackedPaths.length > 0) {
      ...
      fail(
        "agent_mutation_added_unsupported",
        `Added or untracked files are not supported: ${path}.`,
        path
      );
    }

    const rawChanges = parseRawDiff(
      await runGitBuffer(
        workspaceRealPath,
        [
          "diff",
    ...
    if (rawChanges.length === 0) {
      fail("agent_mutation_no_changes", "Agent workspace contains no file changes.");
    }
    if (rawChanges.length > MUTATION_LIMITS.maxFiles) {
      fail(
        "agent_mutation_file_count_exceeded",
        `Agent changed more than ${MUTATION_LIMITS.maxFiles} files.`
      );
    }
    ...
    for (const change of rawChanges) {
      ...
      if (manifestEntry === undefined) {
        fail(
          "agent_mutation_added_unsupported",
          `Changed path did not exist in the pre-agent manifest: ${path}.`,
          path
        );
      }
      ...
      if (!REGULAR_GIT_MODES.has(change.oldMode) || !REGULAR_GIT_MODES.has(change.newMode)) {
        fail(
          change.oldMode === "120000" || change.newMode === "120000"
            ? "agent_mutation_symlink_unsupported"
            : "agent_mutation_file_type_unsupported",
          `Only existing regular files may be converted: ${path}.`,
          path
        );
      }
      if (change.oldMode !== change.newMode) {
        fail(
          "agent_mutation_mode_change_unsupported",
          `File mode changes are not supported: ${path}.`,
          path
        );
      }

      const current = await readCurrentTextFile(
        workspaceRealPath,
        path,
        change.newMode
      );
      totalNewBytes += current.bytes;
      if (totalNewBytes > MUTATION_LIMITS.maxTotalBytes) {
        fail(
          "agent_mutation_total_limit_exceeded",
          `Agent changes exceed ${MUTATION_LIMITS.maxTotalBytes} total bytes.`
        );
      }
    ...
    return Object.freeze({
      mutation: Object.freeze({
        ...mutation,
        claims: Object.freeze([...mutation.claims]),
        touchedFiles: Object.freeze([...mutation.touchedFiles]) as unknown as string[]
      }),
