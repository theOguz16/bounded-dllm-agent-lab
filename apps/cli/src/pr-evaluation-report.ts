...
const reportName = "pr-evaluation-report-v1";
...
type PrEvaluationAggregate = {
  caseCount: number;
  roleCount: number;
  changedFiles: string[];
  prFileCount: number;
  prAdditionCount: number;
  prDeletionCount: number;
  prPatchLineCount: number;
...
type PrEvaluationReport = {
  ...
  aggregate: PrEvaluationAggregate;
  cases: PrEvaluationCaseReport[];
  diagnostics: string[];
};
...
if (fixtureFailures.length) {
  throw new Error(
    JSON.stringify(
      {
        ok: false,
        reason: "Fixture validation failed before PR evaluation.",
        fixtureFailures
      },
      null,
      2
...
async function runReport(): Promise<void> {
  ...
  const jsonPath = join(reportDir, `${safeTimestamp}-pr-evaluation-report.json`);
  const markdownPath = join(reportDir, `${safeTimestamp}-pr-evaluation-report.md`);
...
function summarizeOrchestration(result: OrchestrationRunResult): PrEvaluationCaseReport["orchestration"] {
  return {
...
function summarizeMerge(result: ConflictAwareMergeResult): PrEvaluationCaseReport["merge"] {
  return {
    decision: result.decision,
...
function summarizeRepair(result: RemaskRepairLoopResult): PrEvaluationCaseReport["repair"] {
  return {
...
function createReport(input: {
  ...
  return {
    ...
    reportName,
    createdAt,
    suiteName: "github-style-pr-input-bounded-runtime-evaluation-v1",
    roles,
    prInputPath,
    pr: input.pr,
    changedFiles: input.changedFiles,
    ...
    aggregate: aggregateCases(input.pr, input.repoResult, input.cases, input.changedFiles),
    cases: input.cases,
...
function aggregateCases(
  ...
  return {
    ...
    changedFiles,
    prFileCount: pr.fileCount,
    prAdditionCount: pr.additionCount,
    prDeletionCount: pr.deletionCount,
    prPatchLineCount: pr.patchLineCount,
...
function reportToMarkdown(report: PrEvaluationReport): string {
  const lines: string[] = [];

  lines.push(`# PR Evaluation Report`);
  lines.push("");
  lines.push(`- Report: \`${report.reportName}\``);
  lines.push(`- Created at: \`${report.createdAt}\``);
  lines.push(`- Suite: \`${report.suiteName}\``);
  lines.push(`- PR input: \`${report.prInputPath}\``);
  lines.push(`- Repo: \`${report.pr.repo ?? "(unknown)"}\``);
  lines.push(`- PR number: \`${report.pr.prNumber ?? "(unknown)"}\``);
  lines.push(`- Base ref: \`${report.pr.baseRef ?? "(unknown)"}\``);
  lines.push(`- Head ref: \`${report.pr.headRef ?? "(unknown)"}\``);
  lines.push("");
  ...
  lines.push(`| --- | ---: |`);
  lines.push(`| PR files | ${report.aggregate.prFileCount} |`);
  lines.push(`| Additions | ${report.aggregate.prAdditionCount} |`);
  lines.push(`| Deletions | ${report.aggregate.prDeletionCount} |`);
  lines.push(`| Patch lines | ${report.aggregate.prPatchLineCount} |`);
  lines.push("");
