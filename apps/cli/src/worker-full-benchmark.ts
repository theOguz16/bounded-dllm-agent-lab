...
for (const [index, fixture] of fixtureSubset.entries()) {
  ...
  /**
   * Full benchmark hâlâ fixture packet kullanır.
   * Ama workspace-core packet bilmediği için dönüşüm context-core adapter'dan yapılır.
   */
  const workspace = createWorkspaceFromPacket(fixture.packet, {
    id: `full-benchmark-${fixture.case.id}`
  });
  ...
  /**
   * Full benchmark bütün fixture dataset'ini gerçek worker'a gönderir.
   * Bu artık smoke/dry-run değildir; araştırma hipotezinin dataset geneline yayılıp
   * yayılmadığını görmek için kullanılan ilk tam canlı koşudur.
   */
...
async function writeCheckpoint(checkpoint: FullBenchmarkCheckpoint): Promise<void> {
  ...
  /**
   * Checkpoint her senaryodan sonra yazılır.
   * Uzun GPU koşusunda web terminal, worker veya ağ koparsa aynı run id ile
