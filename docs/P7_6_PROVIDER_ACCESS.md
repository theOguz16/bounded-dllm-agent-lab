...
The command-level smoke executes **the actual `compareCodexCommand`** with an injected fake provider, a local Git fixture and a fake Docker executable that only simulates availability and refuses any other Docker operation. It verifies all four provider failure codes, an unstarted second arm, v2 identity, unknown quota and absence of credential values in the output. Other fake-gate tests check reversed arm order, local account-alias and key switches and mixed-version identity rejection. The P7.6 pull-request workflow uses fake SDKs with provider credentials unset. Offline success is not evidence of live-provider quota, account ownership, or successful real Codex execution.
