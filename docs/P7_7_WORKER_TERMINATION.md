...
The journal stores a task hash, model, deadline, lifecycle observations, bounded redacted failure detail and an explicitly unknown session-evidence field, not the task text or credentials. A completed record requires an observed completed provider result; an interrupted or ambiguous result is never promoted to completed. A later attempt for the same task and stage requires both a different `runId` and an explicit decision identifier bound to the persisted prior `runId`; merely changing the run identifier cannot create a fresh invocation slot.
